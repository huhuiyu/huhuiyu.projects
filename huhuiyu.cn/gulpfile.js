...
const fontsDir = cssLibDir + "fonts/";
...
gulp.task("jsmin", function() {
    ...
    return gulp.src(jsfile).pipe(plumber()).pipe(concat("app.min.js")).pipe(uglify()).pipe(plumber.stop()).pipe(gulp.dest(jsDistDir));
});
...
gulp.task("cssmin", function() {
    ...
    return gulp.src(cssfile).pipe(plumber()).pipe(concat("app.min.css")).pipe(cssmin()).pipe(plumber.stop()).pipe(gulp.dest(cssDistDir));
});
            ...
            if (needIgnore === true) {
                fileSync(s, d, {
                    ignore : function(dir, file) {
                        ...
                        return (synfile == jsDistDir) || (synfile == cssDistDir) || (synfile == imgDistDir) || (synfile == fontsDistDir);
                    }
                });
            } else {
                fileSync(s, d);
